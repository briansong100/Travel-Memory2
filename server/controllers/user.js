...
export const signin = async (req, res) => {
  ...
  try {
    ...
    if (!isPasswordCorrect) return res.status(400).json({ message: "Invalied credentials." })
...
export const signup = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) return res.status(400).json({ message: "Exsisting email, try an other email." })
